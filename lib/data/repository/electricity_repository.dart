...
class ElectricityRepository {
  ...
  ElectricityRepository({required this.accountDao, required this.dailyUsageDao});
  ...
  Future<void> resetCycle(int accountId) async {
    final account = await accountDao.getById(accountId);
    if (account == null) return;
    await accountDao.update(account.copyWith(
      monthlyKwh: 0.0,
      currentSlab: 0,
      slabUsage: 0.0,
      lastUpdated: DateTime.now().millisecondsSinceEpoch,
  ...
  Future<void> simulateDay(int accountId, {double? customKwh}) async {
    ...
    final kwhUsed =
        customKwh ?? (4.0 + Random().nextDouble() * 8.0);

    final oldMonthlyKwh = account.monthlyKwh;
    final newMonthlyKwh = oldMonthlyKwh + kwhUsed;

    final costOfNewTotal =
        CalculationsHelper.calculateCost(newMonthlyKwh, provider: account.distributor);
    final costOfOldTotal =
        CalculationsHelper.calculateCost(oldMonthlyKwh, provider: account.distributor);
    final dailyCost = max(0.0, costOfNewTotal - costOfOldTotal);

    final newBalance = max(0.0, account.balance - dailyCost);

    final slabStats = CalculationsHelper.getSlabDetails(newMonthlyKwh,
        provider: account.distributor);

    await dailyUsageDao.insert(DailyUsageHistory(
      accountId: accountId,
  ...
  Future<String?> syncAccount(int accountId) async {
    ...
    try {
      ...
      final balanceRes = await http.get(balanceUri).timeout(
      ...
      final dateFrom =
          sdf.format(DateTime.now().subtract(const Duration(days: 15)));
      ...
      try {
        final consumptionRes = await http
            .get(consumptionUri)
        ...
        if ((consumptionObj['code'] as int) == 200) {
          final dataArray =
              (consumptionObj['data'] as List).cast<Map<String, dynamic>>();
          ...
          if (list.isNotEmpty) {
            ...
            final currentMonthStr =
                (latestEntry['date'] as String).substring(0, 7);
            ...
            liveMonthlyKwh =
                (liveMonthlyKwh * 1000).roundToDouble() / 1000.0;
          }
          ...
          for (int i = 0; i < list.length; i++) {
            ...
            if (prev != null) {
              dailyKwh = (current['consumedUnit'] as num).toDouble() -
                  (prev['consumedUnit'] as num).toDouble();
              final curMonth =
                  (current['date'] as String).split('-')[1];
            ...
            if (dailyKwh > 0.0 || dailyCost > 0.0) {
              final dateEpoch =
                  sdf.parse(current['date'] as String).millisecondsSinceEpoch;
              final existing =
                  await dailyUsageDao.getRecordForDay(accountId, dateEpoch);
              if (existing != null) {
                await dailyUsageDao.update(existing.copyWith(
                    consumptionKwh: dailyKwh, cost: dailyCost));
              } else {
                await dailyUsageDao.insert(DailyUsageHistory(
                  accountId: accountId,
                  dateEpoch: dateEpoch,
      ...
      final slabStats =
          CalculationsHelper.getSlabDetails(liveMonthlyKwh, provider: 'desco');
